 ...
 * @param limitation
 */
export function isExceedingLimits(velocity: number, transform: number, limitation: Limitation): boolean {
...
export function getShortestWay(
  ...
  const {maxIndex, minIndex} = limitation;
  ...
  const shortcut = (defaultWay > 0 ? 1 : -1) * (minIndex - maxIndex - 1) + targetIndex - currentIndex;
 ...
 * @param limitation
 */
export function getExcess(transform: number, limitation: Limitation): number {
...
export function Operations(
   ...
   * @param force
   */
  function render(duration?: number, cb?: Function, force?: boolean): void {
  ...
  function slideTo(targetIndex: number, duration?: number): void {
    ...
    const boxSize = measure.boxSize;

    let newTransform = -computedIndex * boxSize + limitation.base;
    if (limitation.maxIndex === computedIndex && options.lastForcesGetSize) {
      const $list = env.element.$list;
      newTransform = newTransform + boxSize - $list[$list.length - 1].offsetHeight;
    }

    // const newTransform = -computedIndex * boxSize + limitation.base;
    // console.log('boxSize', newTransform);
    ...
    if (getOffsetSteps(newTransform - state.transforms) !== 0 && options.loop) {
      ...
      const shortcut = getShortestWay(state.index, computedIndex, limitation, defaultWay);
  ...
  function scrollPixel(px: number): void {
    ...
    if (options.loop) {
      ...
      if (excess && isExceedingLimits(velocity, transforms, limitation)) {
        newTransform =
          excess > 0 ? limitation.min - measure.boxSize + excess : limitation.max + measure.boxSize + excess;
      }
    }
