import {addClass, removeClass} from '../core/render/dom';
...
export default <SwiperPlugin>function SwiperPluginPagination(
  ...
  instance.on('after-init', () => {
    const {bulletClass, bulletActiveClass} = <SwiperPluginPaginationOptions>paginationOptions;
    const {element} = instance.env;
    const {$list} = element;
    ...
    for (let index = 0; index < dotCount; index++) {
      ...
      addClass(
        ...
        index === instance.state.index
          ? [...bulletClass.split(/\s+/), ...bulletActiveClass.split(/\s+/)]
          : bulletClass.split(/\s+/),
      );
      $pageList.push($page);
  ...
  instance.on('before-slide', (prevIndex: number, _1: any, currentIndex: number) => {
    const {bulletActiveClass} = paginationOptions;

    paginationInstance.$pageList &&
      paginationInstance.$pageList.forEach(($page, index) => {
        ...
        options.onPaginationChange?.(
          $page,
          paginationInstance.$pageList[prevIndex],
          index === currentIndex,
          currentIndex,
        );
      });
